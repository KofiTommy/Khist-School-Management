...
while($row=mysqli_fetch_array($_SQL_2,MYSQLI_ASSOC)){
	...
	echo "</td>";
	echo "<td>";
	echo $row['subject']." (".semester_registry_session_label(date('Y-m-d H:i:s', strtotime($row['datetimeentry'])), $row['batch'], $row['termname']).")";
	echo "</td>";
	echo "<td align='center'>";
	echo "<a href='upload-class-score-entry.php?class_ID=$row[class_entryid]&term_ID=$row[termname]&batch_ID=$row[batchid]&subject_ID=$row[subjectid]&year_ID=".date('Y', strtotime($row['datetimeentry']))."'><i class='fa fa-plus' style='color:blue'></i></a>";
	echo "</td>";
	echo "</tr>";
	}
echo "</table>";
