...
while($row=mysqli_fetch_array($_SQL_2,MYSQLI_ASSOC)){
	...
	echo "</td>";
	echo "<td align='center'>";
	echo "<a href='upload-exam-score-entry.php?class_ID=$row[class_entryid]&term_ID=$row[termname]&batch_ID=$row[batchid]&subject_ID=$row[subjectid]&year_ID=".date('Y', strtotime($row['datetimeentry']))."'><i class='fa fa-plus' style='color:blue'></i></a>";
	echo "</td>";
	echo "</tr>";
	}
echo "</table>";
